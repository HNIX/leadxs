# Seeds for leads, lead data, and API requests only
# This file creates sample data for testing the API requests and leads tracking interfaces
# It's designed to run independently of other seed files

puts "Creating leads, lead data and API requests..."

# Make sure we have at least one account, campaign, company and vertical to work with
# First get or create an account
if Account.count > 0
  account = Account.first
  puts "Using existing account: #{account.name}"
else
  ...
  puts "Created new account: #{account.name}"
end

# Ensure we're working in the context of the account
ActsAsTenant.with_tenant(account) do
  # Get or create a company
  company = Company.find_by(name: "Test Company") || Company.create!(
    name: "Test Company",
    address: "123 Test St",
    city: "Testville",
    state: "TS",
  ...
  # Get or create a vertical
  vertical = Vertical.find_by(name: "Test Vertical") || Vertical.create!(
    name: "Test Vertical",
  ...
  campaign = Campaign.find_by(name: "Test Campaign") || Campaign.create!(
    name: "Test Campaign",
    status: "active",
